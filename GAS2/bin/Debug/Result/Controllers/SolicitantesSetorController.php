...
use Ciente\Model\VO\SolicitantesSetor;

class SolicitantesSetorController
{
...
public static function getAll (Req $request,Resp $response)
{
            ...
            $query = DBLayer::Connect()->table(SolicitantesSetor::TABLE_NAME)
     ->where(function($query) use($request, $search){
     $query->orWhere(SolicitantesSetor::KEY_ID, DBLayer::OPERATOR_ILIKE, $search);
     $query->orWhere(SolicitantesSetor::ID_SETOR, DBLayer::OPERATOR_ILIKE, $search);
  });
$totalRecords = $query->count();
            $data = $query->orderBy(SolicitantesSetor::KEY_ID, 'asc')
...
public static function save (Req $request,Resp $response)
{
            ...
            try
            {
                ...
                $formData = Utils::filterArrayByArray($request->getParsedBody(), SolicitantesSetor::TABLE_FIELDS);

                if ($idSolicitantesSetor)
                {
                    DBLayer::Connect()->table(SolicitantesSetor::TABLE_NAME)->update($formData);
                }
                else
                {
                    DBLayer::Connect()->table(SolicitantesSetor::TABLE_NAME)->insertGetId($formData);
                }
            }
            catch (Exception $e)
            {
...
public static function get (Req $request,Resp $response)
{
            ...
            try
            {
                ...
                $solicitantesSetor = DBLayer::Connect()->table(SolicitantesSetor::TABLE_NAME)
                    ->where(SolicitantesSetor::KEY_ID,DBLayer::OPERATOR_EQUAL,$idSolicitantesSetor)->first();
...
public static function delete (Req $request,Resp $response)
{
            ...
            try
            {
                ...
                {
                    $solicitantesSetor = DBLayer::Connect()->table(SolicitantesSetor::TABLE_NAME)
                        ->where(SolicitantesSetor::KEY_ID,DBLayer::OPERATOR_EQUAL,$id)->first();

                    if ($solicitantesSetor)
                    {
                        if (DBLayer::Connect()->table(SolicitantesSetor::TABLE_NAME)->delete($id))
                        {
                            $itensDeletadosCount++;
                        }
                    }
